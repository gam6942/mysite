...
def login(mkt_cookie_value,adm_cookie_value):
    ...
    if login == user[0][0] and check(pwhash=user[0][1],password=password) == True and user[0][2] == 1: # marketing
        ...
        with open("html/mkt_1.html","rt") as html: # intégration des composants d'interface suite à l'authentification en marketing
            mkt_1_html = html.read()
        with open("html/mkt_2.html","rt") as html:
            mkt_2_html = html.read()
        with open("html/mkt_3.html","rt") as html:
...
{mkt_3_html}
    ...
    if login == user[0][0] and check(pwhash=user[0][1],password=password) == True and user[0][2] == 0: # accès marketing bloqué
        ...
        with open("html/adm_1.html","rt") as html: # intégration des composants d'interface suite à l'authentification en administrateur
